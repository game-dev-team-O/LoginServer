...
CNetServer::CNetServer(CInitParam* pParam)
{
	...
	TLS_DBConnectIndex = TlsAlloc();
	TLS_DBLastTimeIndex = TlsAlloc();
}
...
DWORD WINAPI CNetServer::AcceptThread(CNetServer* ptr)
{
	...
	while (1)
	{
		...
		ptr->pHandler->OnClientJoin(pSession->sessionID, JoinFlag);
...
DWORD WINAPI CNetServer::WorkerThread(CNetServer* ptr)
{
	...
	TlsSetValue(ptr->TLS_DBConnectIndex, &DBConnector);
	TlsSetValue(ptr->TLS_DBLastTimeIndex, (LPVOID)GetTickCount64());
